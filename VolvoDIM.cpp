...
constexpr int arrSpeed = 0, arrRpm = 1, arrCoolant = 2, arrTime = 3, arrBrakes = 4, arrBlinker = 5, arrAntiSkid = 6, arrAirbag = 7, arr4c = 8, arrConfig = 9, arrGear = 10, arrTrailer = 11, arrDmWindow = 12, arrDmMessage = 13, arrDisplay = 14;
constexpr unsigned long addrLi[listLen] = {0x217FFC, 0x2803008, 0x3C01428, 0x381526C, 0x3600008, 0xA10408, 0x2006428, 0x1A0600A, 0x2616CFC, 0x1017FFC, 0x3200408, 0x3E0004A, 0x02A0240E, 0x1800008, 0x131726C};
/*
   ...
   addrLi[14] = Display Rotate OEM
   */

unsigned char defaultData[listLen][8] = {
    {0x01, 0xEB, 0x00, 0xD8, 0xF0, 0x58, 0x00, 0x00}, // 0, Speed/KeepAlive , 0x217FFC
    {0xFF, 0xE1, 0xFF, 0xFF, 0xFF, 0xCF, 0x00, 0x00}, // 1, RPM/Backlights , 0x2803008
    {0xC0, 0x80, 0x51, 0x89, 0x0E, 0x57, 0x00, 0x00}, // 2, Coolant/OutdoorTemp , 0x3C01428
    ...
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x08, 0x00, 0x00}, // 13 Dim Message Content , 0x1800008
    {0xCF, 0xEB, 0x80, 0xA2, 0xF0, 0xAA, 0x00, 0xAA}, // 14 Display Rotate OEM , 0x131726C
   };
...
void VolvoDIM::setBackBrightness(int value)
{
	if (value >= 0 && value <= 256)
	{
		if (value == 256)
...
void VolvoDIM::enableHighBeam(int enabled)
{
	if (enabled == 1)
	{
		defaultData[arrRpm][1] = (char)0xFF;
	}
	else
	{
		defaultData[arrRpm][1] = (char)0xEA;
	}
}
...
void VolvoDIM::enableBrake(int enabled)
{
	if (enabled == 1)
	{
		defaultData[arrBrakes][3] = (char)0x00;
	}
	else
	{
		defaultData[arrBrakes][3] = (char)0x60;
	}
}
...
void VolvoDIM::enableFog(int enabled)
{
	if (enabled == 1)
	{
		defaultData[arr4c][2] = (char)0xE6;
			}
	else
	{
		defaultData[arr4c][2] = (char)0x00;
			}
}
